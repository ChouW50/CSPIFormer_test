...
class Fusion_CS(nn.Module):
    def __init__(self, channel: int, sp: int, act = nn.ReLU, norm_layer = partial(nn.BatchNorm2d, eps=1e-6)):
        super().__init__()
        self.sp = sp
        self.channel = channel = channel // sp
        self.act = act(inplace = True)
        self.bn = norm_layer(channel)
        self.conv_fus = nn.Conv2d(channel, channel, 3, padding = 1, bias = False)
        
    def fusion_(self, f1, f2):
        fus_ = self.conv_fus(f1 + f2)
        fus_ = self.bn(fus_)
        fus_ = self.act(fus_)
        return fus_
    def forward(self, x):
        ...
        C_ = C // self.sp
        mid_sp = self.sp // 2
        or_part, CS_part = [],[[] for i in range(self.sp)]
        for part in range(self.sp):
            or_part.append(x[:,C_ * part: C_ * (part + 1), :, :])
        # Set 1
        for part in range(self.sp):
            if part == 0 or part == mid_sp:
                CS_part[part] = or_part[(part + mid_sp + (mid_sp - 1)) % self.sp]
                # print(f"CS_part[{part}]: or_part[{(part + mid_sp + (mid_sp - 1)) % self.sp}]")
            elif part == mid_sp - 1 or part == self.sp - 1:
                CS_part[part] = self.fusion_(or_part[(part + 1) % self.sp], or_part[(part + mid_sp) % self.sp])
                # print(f"CS_part[{part}]: fusion or_part[{(part + 1) % self.sp}] and or_part[{(part + mid_sp) % self.sp}]")
            else:
                CS_part[part] = self.fusion_(or_part[(part + (mid_sp - 1)) % self.sp], or_part[(part + mid_sp) % self.sp])
                # print(f"CS_part[{part}]: fusion or_part[{(part + (mid_sp - 1)) % self.sp}] and or_part[{(part + mid_sp) % self.sp}]")
        x = torch.cat(CS_part, dim = 1)
        return x
...
class CSWinAttention(nn.Module):
    ...
    def forward(self, qkv):
        ...
        H = W = self.res
        B, L, C = q.shape
        # print(f"(CSWin)q_before: {q.shape}")
        assert L == H * W, "input feature has wrong size"
        q = self.i2w(q)
        k = self.i2w(k)
        # print(f"(CSWin)q: {q.shape}")
        ...
        x = (attn @ v) + lepe
        x = x.transpose(1, 2)
        # print(f"(CSWin)x: {x.shape}")
        x = x.reshape(-1, self.H_ * self.W_, C) # B', H' * W', C
        # print(f"(CSWin)x_after: {x.shape}")
        x = win2img(x, self.H_, self.W_, H, W) #B, H', W', C
        # print(f"(CSWin)x_final: {x.shape}")
        return x
...
class AxwinLowMixear(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., pool_size=2, win_sizes = (2, 2), res = None, 
        ...
        self.attns_CSWin = nn.ModuleList([
            CSWinAttention(
                self.dim // 2, self.res, idx = i, split_size = self.split_sizes, num_heads = self.h1 // 2, 
                dim_out = self.dim // 2, qk_scale = (dim // self.h1 // 2) ** -0.5, attn_drop = self.attn_drop,
            )for i in range(2)
        ])
    ...
    def forward(self, xa):
        # B, C, H, W
        H = W = self.res
        # print(f"res: {self.res}, dim: {self.dim}")
        xa_ = self.pool(xa)
        # xa_up = self.proj1(xa)
        # xa_dn = self.proj2(xa)
        xa_ = xa_.permute(0, 2, 3, 1)
        # xa_dn = xa_dn.permute(0, 2, 3, 1)
        B, C, _, _ = xa.shape
        # print(f"xa: {xa.shape}, xa_up: {xa_.shape}")
        _, H_, W_, C_ = xa_.shape
        assert H_ * W_ == H * W, "input feature has wrong size"
        qkv_1 = self.qkv(xa_)
        qkv_1 = qkv_1.reshape(B, -1, 3, C_).permute(2, 0, 1, 3)
        # print(f"qkv_1: {qkv_1.shape}")
        x1 = self.attns_CSWin[0](qkv_1[:, :, :, : C // 2]) # row
        x2 = self.attns_CSWin[1](qkv_1[:, :, :, C // 2:]) # column
        line_up = torch.cat([x1, x2], dim = 3)
        ...
        # xa = self.att_fun(q, k, v, B, N, C)
        # xa = xa.view(B, C, int(N**0.5), int(N**0.5))  # .permute(0, 3, 1, 2)
        lower = line_up.view(B, C, H_, W_)
        lower = self.uppool(lower)
        return lower
class Inception_token_mixer(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., attention_head=1, pool_size=2, 
                    win_sizes = None, split_sizes = None, res = None,
                    **kwargs, ):
        super().__init__()
        self.num_heads = num_heads
        self.head_dim = head_dim = dim // num_heads
...
class IFormer_Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop_rate=0., attn_drop=0.,
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim,
                        act_layer=act_layer, drop=drop_rate)
...
class CSP_IFomerBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio, qkv_bias, drop_rate, attn_drop_rate, dpr, norm_layer, act_layer, 
        ...
        # attn: Inception_token_mixer, CSP_Inception_token_mixer
        self.dense = nn.Sequential(*[
            IFormer_Block(
                dim=self.part1_chnls, num_heads=self.first_num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop_rate=drop_rate,
        ...
        self.f_CS = Fusion_CS(channel = self.part1_chnls + self.part2_chnls, sp = 4)
    def forward(self, x):
        ...
        # ----channel shuffle end----
        return x
class TwoPart_(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio, qkv_bias, drop_rate, attn_drop_rate, dpr, norm_layer, act_layer, attention_heads, 
                    pool_size, use_layer_scale, layer_scale_init_value, idx1, idx2, win_sizes = None, split_sizes = None, res = None, 
                    part_ratio = 0.5, dense = CSP_IFomerBlock, **kwargs):
        super(TwoPart_, self).__init__()
        self.part1_chnls = int(dim * part_ratio)
        self.part2_chnls = dim - self.part1_chnls
        self.first_num_heads = int(num_heads * part_ratio)
...
class CSP_InceptionT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=None, depths=None,
                    ...
                    checkpoint_path=None, win_sizes = None, split_sizes = None, part_ratio = 0.5, dense = CSP_IFomerBlock, 
                    **kwargs
                    ):
        super().__init__()
        ...
        self.blocks1 = TwoPart_(embed_dims[0], num_heads[0], mlp_ratio, qkv_bias, drop_rate, attn_drop_rate, dpr, 
                                    norm_layer, act_layer, attention_heads, 2, False, 1e-5, 0, st2_idx, win_sizes, 
                                    split_sizes, res = img_size // 4)
        ...
        self.blocks2 = TwoPart_(embed_dims[1], num_heads[1], mlp_ratio, qkv_bias, drop_rate, attn_drop_rate, dpr, 
                                    norm_layer, act_layer, attention_heads, 2, False, 1e-5, st2_idx, st3_idx)
        ...
        self.blocks3 = TwoPart_(embed_dims[2], num_heads[2], mlp_ratio, qkv_bias, drop_rate, attn_drop_rate, dpr, 
                                    norm_layer, act_layer, attention_heads, 1, use_layer_scale, layer_scale_init_value, 
                                    st3_idx, st4_idx)
        ...
        self.blocks4 = TwoPart_(embed_dims[3], num_heads[3], mlp_ratio, qkv_bias, drop_rate, attn_drop_rate, dpr, 
                                    norm_layer, act_layer, attention_heads, 1, use_layer_scale, layer_scale_init_value, 
                                    st4_idx, depth)
...
def iformer_small(pretrained=False, size=256, in_channel=3, **kwargs):
    ...
    # CSP_InceptionT, InceptionTransformer
    model = CSP_InceptionT(img_size=size,
                                in_chans=in_channel,
                                depths=depths,
