...
class CSWinAttention(nn.Module):
    ...
    def forward(self, qkv):
        ...
        CS_attn = (q @ k.transpose(-2, -1)) * self.scale
        # print(f"(CSWin)attn: {CS_attn.shape}")
        CS_attn = CS_attn.softmax(dim=-1)
        CS_attn = self.attn_drop(CS_attn)
        # x = (CS_attn @ v) + lepe
        x = (CS_attn @ v)
        x = x.transpose(1, 2)
...
class AxwinLowMixear(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., pool_size=2, win_sizes = (2, 2), res = None, 
        ...
        super().__init__()
        # num_head: [1]*3, [3]*3, [7]*4... h1 = 4(for 2 lines) or 2(only for Ax line)
        if num_heads == 1:
            self.h1 = 4
        # print(f"Ax_head: {self.h1}")
        # elif num_heads == 3:
        #     self.h1, self.h2 = 4, 6
